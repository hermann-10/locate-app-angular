...
export class OffersService {
  ...
  getOffers(): void{
    this.db.list('offers').query.limitToLast(10).once('value', snapshot => {
      ...
      console.log('OBJECT >>>>', offersSnapshotValue);
      const offers = Object.keys(offersSnapshotValue).map(id => ({id, ...offersSnapshotValue[id]})); //tranform in array
      console.log('ARRAY >>>>', offers);
      this.offers = offers;
      this.dispatchOffers(); //emet via l'observable le nouvel état du tableau (à tous les composants qui sont abonnés)
    });
  }
  ...
  private uploadPhoto(photo: any): Promise<any>{
    return new Promise ((resolve, reject) => {
      const upload = this.storage.upload('offers/'+ Date.now().toString() + '-' + photo.name, photo);
      upload.then((res) => {
